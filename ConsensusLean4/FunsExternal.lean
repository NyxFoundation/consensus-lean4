...
-- Fill the `axiom` holes below with real Lean implementations to make the model
-- computable. `FunsExternal_Template.lean` is refreshed on every Aeneas run and
-- remains the canonical signature reference — diff it against this file to catch
